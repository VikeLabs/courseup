...
export const getCourseSections = functions.https.onRequest(async (req, res) => {
  ...
  if (!assertTermValid(res, term)) {
    return;
  }

  try {
    const courseSections: ClassScheduleListing[] = await UVicCourseScraper.getCourseSections(
      term,
      subject,
      code
    );
    res.send(
      courseSections
        ? courseSections
        : 'No sections found for ' + subject + code + ' in ' + term
    );
  } catch (err) {
    sendInternalError(res, err);
  }
  ...
  try {
    const crnToSeatsMap: DetailedClassInformation[] = await Promise.all(
      snapshot.val().map(async (crn: any) => {
        const details: DetailedClassInformation = await UVicCourseScraper.getSectionSeats(
          term,
          crn
        );
        return { crn, ...details };
